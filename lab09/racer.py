...
while True:
    ...
    if coin_rect.colliderect(player_rect) and not coin_collected:
        coin_weight = randrange(1,4)
        point += coin_weight
        ncoin +=1
        #increase enemy speed 
        enemy_speed+=point*0.25
        coin_collected = True  
        coin_rect.x = -coin_rect.width

    #render the text of score and number of coins
    score = pg.font.Font(None, 36)
    score_text = score.render(f"Score: {point}", True, (0,0,0))
    coin_text = score.render(f"Coins: {ncoin}", True, (0,0,0))
    text_pos = score_text.get_rect(topright=screen.get_rect().topright)
    ...
    screen.blit(coin_text, (text_pos[0], text_pos[1]+36))
    pg.display.update()
    clock.tick(60)
